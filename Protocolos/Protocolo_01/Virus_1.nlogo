globals [path-color internal-color external-color lake-center-x lake-center-y lake-size lake-radius healthy-color infected-color imune-color total-infections imune-prob death-prob deaths cure-length cure-prob agents-cured agents-imune spread-radius weather-status weather-prob]
breed [humans human]
breed [weathers weather]
humans-own [infected p_r0 imune virus p_inf cure_ticks]
...
to setup
  ...
  set cure-length 21 ;  Número mínimo de dias necessários para que um agente seja capaz de se curar.
  set cure-prob 0.30 ; Probabilidade de um agente se auto-curar após o número mínimo de dias passar.
...
to spread
  ...
  ifelse (any? humans with [virus = "saudável" and imune = false])[
    ask one-of humans[
    ...
    if(random-float 1 <= p_inf)[
      ...
      if any? other humans in-radius spread-radius[
        ask other humans in-radius spread-radius[
          ; Os outros agentes apenas são infetados se não forem imunes à doença.
